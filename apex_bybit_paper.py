...
def calc_supertrend(candles, period=10, multiplier=3.0):
    """
    Returns (latest_supertrend, is_bullish) where is_bullish=True if price > supertrend.
    ...
    # final supertrend logic (rolling forward)
    supertrend = [0.0] * len(candles)
    direction = [1] * len(candles)   # 1 for up (bull), -1 for down (bear)
...
def calc_ut_bot(candles, key_value=1.0, atr_period=10):
    """
    UT Bot Alerts: uses Heikin-Ashi candles + ATR trailing stop.
    Returns (ut_buy_signal, ut_sell_signal) booleans for the latest candle.
    """
    ...
    # ATR on Heikin-Ashi (or original candles, using original for simplicity)
    ...
    for i in range(1, len(candles)):
        ...
        if ha_close[i-1] > prev_stop:   # uptrend
...
def get_ta(symbol):
    """Fetch candles and compute new indicators: Supertrend, UT Bot, EMAs."""
    ...
    atr_val, atr_pct = calc_atr(candles, 14)   # kept for SL calculation
...
def build_signal(price, change, high, low, vol, ta):
    """
    New engine: Supertrend + UT Bot + EMA alignment.
    Long  = UT buy signal AND supertrend bullish AND EMA20 > EMA50 > EMA200
    Short = UT sell signal AND supertrend bearish AND EMA20 < EMA50 < EMA200
    Confidence based on agreement (2-3 out of 3).
    """
    ...
    if ut_buy and super_bull and uptrend_ema:
        ...
        direction = "SELL"
        agreements = 3
    elif ut_buy and super_bull:   # missing EMA
        direction = "BUY"
        agreements = 2
    ...
    # Confidence based on agreements (scaled to 70-95)
    conf = 70 + agreements * 10

    # Levels using ATR for dynamic TP/SL (keep same logic as before)
    atr_val = ta.get("atr")
    atr_pct = ta.get("atr_pct")

    # TP levels as fractions of ATR (unchanged, but now scaled from ATR if available)
    ...
    return {
        ...
        "tp_pcts":  tp_pcts,
        "tp1_prob": 80,    # simplified probabilities
        "tp2_prob": 65,
    }
...
def run():
    ...
    tg_send(f"<b>⚡ APEX — PHASE 2 (Supertrend + UT Bot + EMA)</b>\n\n"
