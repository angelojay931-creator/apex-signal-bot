...
def calc_valid_limit_buy(price, qdec):
price_dec = dec(price)
if price_dec <= 0:
return None, “Invalid price”
price_str = floor_to_decimals(price_dec, 2)
price_dec = dec(price_str)
raw_qty = TRADE_SIZE / price_dec
qty_str = floor_to_decimals(raw_qty, qdec)
qty = dec(qty_str)
if qty <= 0:
return None, “Calculated quantity is zero”
notional = qty * price_dec
print(f”Order: price={price_str} qty={qty_str} notional={notional}”)
return {“qty”: qty, “qty_str”: qty_str, “price_str”: price_str, “notional”: notional}, None
...
if action == "CONFIRM" and sym in pending:
    ...
    calc, err = calc_valid_limit_buy(price=order_price, qdec=trade["qdec"])
    if err:
...
def run():
print(“APEX Bot 25 Coins started”)
