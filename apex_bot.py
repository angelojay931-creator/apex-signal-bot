...
pending = {}
positions = {}
watching = {}  # tracks skipped/expired signals for result notifications
symbol_rules_cache = {}
...
pass
return {“qty”: qty, “qty_str”: qty_str, “price_str”: price_str, “notional”: notional, “pdec”: pdec, “qdec”: qdec}, None
...
“binancecoin”: “BNBUSDT”, “solana”: “SOLUSDT”, “dogecoin”: “DOGEUSDT”,
“cardano”: “ADAUSDT”, “tron”: “TRXUSDT”, “avalanche-2”: “AVAXUSDT”,
“sui”: “SUIUSDT”, “chainlink”: “LINKUSDT”, “stellar”: “XLMUSDT”,
“litecoin”: “LTCUSDT”, “polkadot”: “DOTUSDT”, “uniswap”: “UNIUSDT”,
“near”: “NEARUSDT”, “aptos”: “APTUSDT”, “internet-computer”: “ICPUSDT”,
“ethereum-classic”: “ETCUSDT”, “filecoin”: “FILUSDT”,
“injective-protocol”: “INJUSDT”, “optimism”: “OPUSDT”,
“arbitrum”: “ARBUSDT”, “pepe”: “PEPEUSDT”, “shiba-inu”: “SHIBUSDT”,
}
...
“”“Monitor skipped/expired signals and notify when TP or SL is hit.”””
to_remove = []
now = time.time()
...
for sym, w in watching.items():
    ...
    # Expire after 24 hours without hitting TP or SL
    if elapsed > 86400:
        pct = round((price - entry) / entry * 100, 2)
        tg_send(
            f"<b>SIGNAL EXPIRED (Not Confirmed)</b>\n\n"
            f"{sym} {sig_type} signal expired after {elapsed_str(elapsed)}\n"
            f"Entry: ${entry}\n"
            f"Current: ${round(price, 6)}\n"
            f"Change from entry: {'+' if pct >= 0 else ''}{pct}%\n\n"
            f"TP: ${tp} | SL: ${sl}"
        )
        to_remove.append(sym)
        continue
    ...
    if sig_type == "BUY":
        if price >= tp:
            ...
            tg_send(
                ...
                f"Entry: ${entry}\n"
                f"TP Hit: ${tp}\n"
                f"Time to hit: {elapsed_str(elapsed)}\n\n"
...
if action == "CONFIRM" and sym in pending:
    ...
    watching.pop(sym, None)  # remove from watching if confirmed
    ...
    # Add to watching so we track the outcome
    if trade:
        watching[sym] = {
            "signal": trade["signal"],
            "entry": trade["price"],
    ...
    # Signal expired - add to watching
    if sym in pending:
        trade = pending.pop(sym)
        watching[sym] = {
            "signal": trade["signal"],
...
while True:
    try:
        ...
        prices = get_prices()
        if prices:
            # Monitor watching (skipped signals)
            if watching:
        ...
        if time.time() - last_scan_at >= SCAN_EVERY_SECONDS:
            ...
            if prices:
                for coin in COINS:
                    ...
                    # Auto add to watching immediately
                    watching[sym] = {
                        "signal": sig["signal"],
                        "entry": price,
                        "tp": sig["tp"],
