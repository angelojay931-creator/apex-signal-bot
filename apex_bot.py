...
def fmt_decimal_clean(d: Decimal) -> str:
    s = format(d, "f")
    if "." in s:
        s = s.rstrip("0").rstrip(".")
...
def floor_to_step(value, step) -> str:
    v = dec(value)
    step_dec = dec(step)
    if step_dec <= 0:
...
def calc_valid_limit_buy(symbol, price, fallback_pdec, fallback_qdec):
    ...
    print(
        f"Order validated: symbol={symbol} price={price_str} qty={qty_str} "
        f"notional={fmt_decimal_clean(notional)} pdec={pdec} qdec={qdec}"
    )
    ...
    if action == "CONFIRM" and sym in pending:
        ...
        if res and res.get("result"):
            ...
            tg_send(
                ...
                f"Qty: {calc['qty_str']}\n"
                f"Notional: ${round(float(calc['notional']), 6)}\n"
                f"TP: ${trade['tp']}\n"
                f"SL: ${trade['sl']}\n"
                f"Order ID: {order_id}\n\n"
                "Monitoring..."
...
def preload_symbol_rules():
    ...
    for coin in COINS:
        try:
            rules = px_get_symbol_rules(coin["pionex"])
            if rules:
                print(
                    f"{coin['pionex']} rules: "
                    f"baseStep={rules.get('baseStep')} "
                    f"quoteStep={rules.get('quoteStep')} "
                    f"minNotional={rules.get('minNotional')} "
                    f"minSizeLimit={rules.get('minSizeLimit')}"
                )
            time.sleep(0.05)
        except Exception as e:
