...
# qdec = max decimal places for quantity on Pionex
COINS = [
...
def calc_valid_limit_buy(symbol, price, qdec):
    price_dec = dec(price)
    if price_dec <= 0:
        return None, "Invalid price"

    # Format price to 2 decimal places (Pionex standard for USDT pairs)
    price_str = floor_to_decimals(price_dec, 2)
    price_dec = dec(price_str)

    raw_qty = TRADE_SIZE / price_dec

    # Use coin's qdec for quantity precision
