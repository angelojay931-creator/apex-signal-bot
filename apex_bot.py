...
def calc_valid_limit_buy(symbol, price):
    ...
    # Ceil to precision so it never slips below min
    ...
    # Use floor_to_decimals for qty_str to preserve trailing zeros
    qty_str = floor_to_decimals(qty, amount_precision)
...
def handle(cb):
    ...
    if action == "CONFIRM" and sym in pending:
        ...
        # Get live book ticker price for accurate limit order
        ...
        calc, err = calc_valid_limit_buy(symbol=trade["pionex"], price=order_price)
        if err:
