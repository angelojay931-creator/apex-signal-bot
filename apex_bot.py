...
def ceil_to_decimals(value, decimals):
if decimals < 0:
decimals = 0
quant = Decimal(“1”) if decimals == 0 else Decimal(“1.” + (“0” * decimals))
...
params = {“timeout”: 1, “allowed_updates”: json.dumps([“callback_query”])}
...
headers = {“PIONEX-KEY”: PX_KEY, “PIONEX-SIGNATURE”: signature, “Content-Type”: “application/json”}
url = f”https://api.pionex.com{path}”
try:
if method.upper() == “GET”:
...
def calc_valid_limit_buy(symbol, price):
...
qty_str = floor_to_decimals(raw_qty, amount_precision)
qty = dec(qty_str)
if qty < min_trade_size:
qty = min_trade_size
qty_str = ceil_to_decimals(qty, amount_precision)
qty = dec(qty_str)
notional = qty * price_dec
if notional < min_amount:
needed_qty = min_amount / price_dec
qty_str = ceil_to_decimals(needed_qty, amount_precision)
qty = dec(qty_str)
notional = qty * price_dec
if qty <= 0:
return None, “Calculated quantity is too small”
return {“qty”: qty, “qty_str”: qty_str, “notional”: notional, “amount_precision”: amount_precision, “min_amount”: min_amount, “min_trade_size”: min_trade_size}, None
...
calc, err = calc_valid_limit_buy(symbol=trade[“pionex”], price=trade[“price”])
if err:
tg_send(f”<b>Order failed</b>\n\n{err}”)
return
price_str = floor_to_decimals(trade[“price”], 8)
qty_str = calc[“qty_str”]
res = px_order_limit_buy(symbol=trade[“pionex”], price_str=price_str, qty_str=qty_str)
if res and res.get(“result”):
data_obj = res.get(“data”, {}) or {}
order_id = data_obj.get(“orderId”)
positions[sym] = {“entry”:trade[“price”],“tp”:trade[“tp”],“sl”:trade[“sl”],“qty”:float(calc[“qty”]),“order_qty_str”:qty_str,“order_id”:order_id,“pionex”:trade[“pionex”]}
tg_send(
“<b>ORDER PLACED!</b>\n\n”
f”{sym}/USDT BUY @ {price_str}\n”
f”Qty: {qty_str}\n”
...
sig[“entry”] = price
last_signal[sym] = sig
pending[sym] = {“signal”:sig[“signal”],“price”:price,“tp”:sig[“tp”],“sl”:sig[“sl”],“pionex”:coin[“pionex”]}
markup = {“inline_keyboard”:[[{“text”:“CONFIRM TRADE”,“callback_data”:f”CONFIRM_{sym}”},{“text”:“SKIP”,“callback_data”:f”SKIP_{sym}”}]]}
