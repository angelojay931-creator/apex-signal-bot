...
def format_decimal_clean(value) -> str:
    """
    Convert Decimal/number to plain string without scientific notation.
    """
...
def calc_valid_limit_buy(symbol: str, price):
    """
    Build a valid LIMIT BUY quantity based on symbol metadata.

    Uses:
    - amountPrecision  -> qty decimal places
    - minTradeSize     -> minimum base size
    - minAmount        -> minimum quote notional
    """
    ...
    # start from desired spend
    raw_qty = TRADE_SIZE / price_dec

    # round DOWN first to allowed size precision
    qty = dec(floor_to_decimals(raw_qty, amount_precision))

    # if rounded qty becomes zero, bump to one precision step
    if qty <= 0:
    ...
    # IMPORTANT:
    # after setting minTradeSize, force it to allowed precision using CEIL,
    # not floor, so it never drops below the min after formatting
    ...
    # final safety pass: still respect minTradeSize after formatting
    if qty < min_trade_size:
        qty = dec(ceil_to_decimals(min_trade_size, amount_precision))

    qty_str = format_decimal_clean(qty)
    price_str = format_decimal_clean(dec(price))
