...
TRADE_SIZE = Decimal("15")   # safer than 10 for min amount filters
MIN_CONF = 65
...
def calc_valid_limit_buy(symbol: str, price):
    """
    Uses exchange symbol metadata to create a qty that respects:
    - amountPrecision
    - minTradeSize
    - minAmount
    """
...
def run():
    ...
    while True:
        try:
            ...
            # monitor positions for alert only
            if positions:
                prices = get_prices()
                if prices:
                    for coin in COINS:
